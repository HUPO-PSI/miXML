...
<!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
<!-- ** Written by Matthias Oesterheld @ MIPS (mips.gsf.de) **                    -->
...
<xsl:template match="psi:entry/psi:availabilityList">
	<xsl:copy>
		...
		<xsl:for-each select="../psi:interactionList/psi:interaction/psi:availability">
			<xsl:choose>
				<xsl:when test="../preceding-sibling::node()/psi:availability/node()[@id=current()/@id]"/>
				<xsl:otherwise>
...
<xsl:template match="psi:entry/psi:experimentList">
	<xsl:copy>
		...
		<xsl:for-each select="../psi:interactionList/psi:interaction/psi:experimentList/psi:experimentDescription">
			<xsl:choose>
				<xsl:when test="../../preceding-sibling::node()/psi:experimentList/node()[@id=current()/@id]"/>
				<xsl:when test="preceding-sibling::node()[@id=current()/@id]"/>
				<xsl:otherwise>
					<xsl:copy><xsl:apply-templates select="@* | node()"/></xsl:copy>				
				</xsl:otherwise>
			</xsl:choose>
...
<xsl:template match="psi:entry/psi:interactorList">
	<xsl:copy>
		<!-- copy every participant from the interactionList-->
		<xsl:for-each select="../psi:interactionList/psi:interaction/psi:participantList/psi:participant/psi:interactor">
			<xsl:choose>
				<xsl:when test="../../../preceding-sibling::node()/psi:participantList/psi:participant/psi:interactor[@id=current()/@id]"/>
				<xsl:when test="../preceding-sibling::node()/node()[@id=current()/@id]"/>
				<xsl:otherwise>
					<xsl:copy><xsl:apply-templates select="@* | node()"/></xsl:copy>				
				</xsl:otherwise>
			</xsl:choose>
...
<!-- Create a entry in the global interactorList for each participant in the interactionList (used if entry/interactorList had to be created) -->
...
<xsl:template match="psi:interaction/psi:experimentList/psi:experimentDescription">
	<xsl:element name="experimentRef"><xsl:value-of select="@id"/></xsl:element>	
</xsl:template>
...
<xsl:template match="psi:interaction/psi:availability">
	<xsl:element name="availabilityRef"><xsl:value-of select="@id"/></xsl:element>
</xsl:template>
	...
	<xsl:copy>
		<xsl:for-each select="node()">
			<xsl:copy>
				...
				<xsl:element name="interactorRef"><xsl:value-of select="@id"/></xsl:element>
			</xsl:copy>
		</xsl:for-each>
	</xsl:copy>
</xsl:template>
