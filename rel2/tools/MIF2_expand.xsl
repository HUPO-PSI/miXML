...
<!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
<!-- ** Modified for MIF version 2 by Matthias @ MIPS (mips.gsf.de) ** -->
...
<xsl:template match="psi:availabilityRef">
  <xsl:element name="availabilityDescription">
    ...
    <xsl:value-of select="../../../psi:availabilityList/psi:availability[@id=current()/@ref]"/>
  </xsl:element>
</xsl:template>
...
<xsl:template match="psi:experimentRef">
  <xsl:copy-of select="../../../../psi:experimentList/psi:experimentDescription[@id=current()/@ref]"/>
</xsl:template>

<!-- We replace an interactorRef node by the interactorDescription  -->
<!-- node whose id attribute is the same as the ref attribute here. -->
<xsl:template match="psi:proteinInteractorRef">
  <xsl:copy-of select="../../../../../psi:interactorList/psi:proteinInteractor[@id=current()/@ref]"/>
</xsl:template>
